...
namespace TechStore.Controllers
{
    public class CartController : Controller
    {
        ...
        public async Task<IActionResult> PaymentCart(Payment model)
        {
            ...
            { 
                ModelState.AddModelError("Customers.NameCus", nameCheck.Error);
                isValid = false; 
            }

            // Cập nhật thông tin từ form vào cus trước khi validate
            if (!string.IsNullOrWhiteSpace(model.Customers?.StreetAddress))
                cus.StreetAddress = model.Customers.StreetAddress;
            if (!string.IsNullOrWhiteSpace(model.Customers?.Ward))
                cus.Ward = model.Customers.Ward;
            if (!string.IsNullOrWhiteSpace(model.Customers?.City))
                cus.City = model.Customers.City;
            if (!string.IsNullOrWhiteSpace(model.Customers?.PhoneCus))
                cus.PhoneCus = model.Customers.PhoneCus;
            if (!string.IsNullOrWhiteSpace(model.Customers?.EmailCus))
                cus.EmailCus = model.Customers.EmailCus;
       ...
       private (bool IsValid, string Error) ValidateName(string name)
        ...
        private (bool IsValid, string Error) ValidateAddress(string address)
        {
            if (string.IsNullOrWhiteSpace(address))
                return (false, "Địa chỉ không được để trống. Vui lòng nhập số nhà, đường, phường và thành phố.");
